...
const ARCHETYPE_META: Record<
  Archetype,
  { label: string; emoji: string; zone: Zone; pill: string }
> = {
  cheerleader: { label: "夸夸迷妹", emoji: "🌸", zone: "encourage", pill: "bg-pink-50 border-pink-200 text-pink-700" },
  buddy: { label: "互吹搭子", emoji: "✨", zone: "encourage", pill: "bg-amber-50 border-amber-200 text-amber-700" },
  ...
  critic: { label: "毒舌老读者", emoji: "🎯", zone: "feedback", pill: "bg-stone-50 border-stone-200 text-stone-600" },
  custom: { label: "自定义", emoji: "⭐", zone: "feedback", pill: "bg-purple-50 border-purple-200 text-purple-700" },
};
...
export default function ReadersPage() {
  ...
  useEffect(() => {
    ...
    const bid = urlBookId && allBooks.find((b) => b.id === urlBookId)
      ? urlBookId
      : allBooks[0]?.id ?? "";
    setActiveBookId(bid);
  ...
  function handleAddReader() {
    ...
    const r: Reader = {
      ...
      customPersonality: fCustom,
      readingProgress: fProgress,
      createdAt: new Date().toISOString(),
    };
    const nextReaders = [...readers, r];
    setReaders(nextReaders);
    persistAll(nextReaders, messages);
    setShowForm(false);
  ...
  function handleDelete(id: string) {
    ...
    setReaders(nextR); setMessages(nextM);
    persistAll(nextR, nextM);
  }
  ...
  async function handleSummon(reader: Reader) {
    ...
    const personality = reader.archetype === "custom" && reader.customPersonality
      ? reader.customPersonality
      : PERSONALITY_PROMPTS[reader.archetype];
    ...
    try {
        ...
        for (const line of dec.decode(value, { stream: true }).split("\n")) {
          ...
          try {
            ...
            if (delta) {
              ...
              setMessages((prev) =>
                prev.map((m) => m.id === msgId ? { ...m, content: streamRef.current } : m)
              );
            }
          } catch {}
        }
      ...
      setMessages((prev) =>
        prev.map((m) => m.id === msgId ? { ...m, content: "（生成失败，请检查模型设置）" } : m)
      );
    } finally {
      setMessages((prev) => {
        const updated = prev.map((m) =>
          m.id === msgId ? { ...m, content: streamRef.current || "（生成失败）" } : m
        );
  ...
  if (!activeBookId) {
    return (
      <div className="flex min-h-screen items-center justify-center text-[#9b744d]">
  ...
  return (
    <div className="min-h-screen bg-[#f8f0df] text-[#4f3524]">
      {/* Header */}
      <header className="border-b border-[#e0c9a5] bg-[#fff8eb]/90 px-6 py-4">
        <div className="mx-auto flex max-w-3xl items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/" className="text-sm text-[#9b744d] transition hover:text-[#4f3524]">← 回到小屋</Link>
            {activeBook && <><span className="text-[#d8b98f]">/</span><span className="text-sm text-[#9b744d]">{activeBook.title}</span></>}
          </div>
          {books.length > 1 && (
            <select value={activeBookId} onChange={(e) => setActiveBookId(e.target.value)}
              className="rounded-lg border border-[#e0c9a5] bg-[#fff8eb] px-3 py-1.5 text-sm text-[#4f3524] outline-none">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-[#4f3524]">📬 读者来信</h1>
            <p className="mt-1 text-sm text-[#9b744d]">
        ...
        {/* Add reader form */}
        {showForm && (
          <div className="mb-6 rounded-2xl border border-[#e0c9a5] bg-white p-5 shadow-sm">
            <p className="mb-4 text-sm font-medium text-[#9b744d]">新读者档案</p>

            <div className="mb-3">
              <label className="mb-1.5 block text-xs text-[#9b744d]">头像</label>
              <div className="flex flex-wrap gap-2">
                {AVATARS.map((emoji) => (
                  <button key={emoji} onClick={() => setFAvatar(emoji)}
                    className={`h-9 w-9 rounded-xl text-lg transition ${fAvatar === emoji ? "bg-[#6e4b2d] ring-2 ring-[#6e4b2d]" : "bg-[#f8f0df] hover:bg-[#e0c9a5]"}`}>
                    {emoji}
                  </button>
                ))}
              </div>
            ...
            <div className="mb-3">
              <label className="mb-1.5 block text-xs text-[#9b744d]">昵称</label>
              <input value={fName} onChange={(e) => setFName(e.target.value)}
                placeholder="给这位读者起个名字"
                className="w-full rounded-xl border border-[#e0c9a5] bg-[#fff8eb] px-3 py-2 text-sm text-[#4f3524] outline-none focus:border-[#9b744d]" />
            </div>

            <div className="mb-3">
              <label className="mb-1.5 block text-xs text-[#9b744d]">人格类型</label>
                ...
                {(Object.keys(ARCHETYPE_META) as Archetype[]).map((a) => (
                  <button key={a} onClick={() => setFArchetype(a)}
                    className={`rounded-xl border px-2 py-2 text-xs transition ${
                      fArchetype === a ? "border-[#6e4b2d] bg-[#6e4b2d] text-amber-50" : "border-[#e0c9a5] bg-[#fff8eb] text-[#4f3524] hover:border-[#9b744d]"
                    }`}>
            ...
            {fArchetype === "custom" && (
              <div className="mb-3">
                <label className="mb-1.5 block text-xs text-[#9b744d]">自定义人格描述</label>
                <textarea value={fCustom} onChange={(e) => setFCustom(e.target.value)} rows={2}
                  placeholder="描述这位读者的性格和留言风格……"
                  className="w-full resize-none rounded-xl border border-[#e0c9a5] bg-[#fff8eb] px-3 py-2 text-sm text-[#4f3524] outline-none focus:border-[#9b744d]" />
              </div>
            )}

            {bookChapters.length > 0 && (
              <div className="mb-4">
                <label className="mb-1.5 block text-xs text-[#9b744d]">阅读进度（读到哪章）</label>
                <select value={fProgress} onChange={(e) => setFProgress(Number(e.target.value))}
                  ...
                  {bookChapters.map((ch, i) => (
                    <option key={ch.id} value={i}>第{i + 1}章：{ch.title}</option>
                  ))}
                </select>
              </div>
            )}
        ...
        {bookReaders.length > 0 && (
          <div className="mb-6 flex gap-3 overflow-x-auto pb-2">
            {bookReaders.map((reader) => {
              ...
              return (
                <div key={reader.id}
                  className="flex min-w-[158px] flex-col rounded-2xl border border-[#e0c9a5] bg-white p-4 shadow-sm">
                  <div className="flex items-start justify-between">
                    <span className="text-3xl">{reader.avatar}</span>
                    <button onClick={() => handleDelete(reader.id)}
                      className="text-xs text-[#d8b98f] transition hover:text-[#9b744d]">✕</button>
                  </div>
                  <p className="mt-2 text-sm font-medium text-[#4f3524]">{reader.name}</p>
                  <span className={`mt-1 self-start rounded-full border px-2 py-0.5 text-xs ${meta.pill}`}>
                    {meta.emoji} {meta.label}
                  </span>
                  <p className="mt-1.5 text-xs text-[#c7a984]">
                    {bookChapters[reader.readingProgress] ? `读到第${reader.readingProgress + 1}章` : "还没开始读"}
                  </p>
                  {reader.archetype === "urger" && (
                    <p className="mt-0.5 text-xs text-orange-400">连续催更 {daysSince(reader.createdAt)} 天</p>
                  )}
        ...
        {/* Zone tabs */}
        <div className="mb-4 flex gap-1 rounded-2xl border border-[#e0c9a5] bg-[#fff8eb] p-1">
          {(Object.keys(ZONE_META) as Zone[]).map((zone) => (
            <button key={zone} onClick={() => setActiveZone(zone)}
              className={`flex-1 rounded-xl py-2 text-sm transition ${
                activeZone === zone ? "bg-[#6e4b2d] text-amber-50 shadow-sm" : "text-[#9b744d] hover:bg-white"
              }`}>
        ...
        <div className="space-y-4">
          {zoneMessages.length === 0 ? (
            <div className="rounded-2xl border border-[#e0c9a5] bg-white/60 p-10 text-center">
              <p className="text-sm text-[#c7a984]">{ZONE_META[activeZone].hint}</p>
            </div>
          ) : (
            zoneMessages.map((msg, idx) => (
              <div key={msg.id} className="rounded-2xl border border-[#e0c9a5] bg-white p-5 shadow-sm">
                <div className="mb-3 flex items-center gap-3">
                  <span className="text-2xl">{msg.readerAvatar}</span>
                  <div>
                    <p className="text-sm font-medium text-[#4f3524]">{msg.readerName}</p>
                    <p className="text-xs text-[#c7a984]">{timeAgo(msg.createdAt)}</p>
                  </div>
                </div>
                <p className="whitespace-pre-wrap text-sm leading-7 text-[#4f3524]">
                  {msg.content}
                  {generatingId !== null && idx === 0 && (
