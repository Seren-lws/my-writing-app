...
export default function ModelSettingsPage() {
  ...
  /* ── 暖浅色调（与灵魂卡 / 写作 DNA 统一）── */
  const inputClass = "w-full rounded-xl border border-[#e0d4c0] bg-white/70 px-4 py-2.5 text-sm text-[#3d2b1a] outline-none placeholder:text-[#c0a078] focus:border-[#b8893f] transition";
  ...
  const saveActions = (
    <div className="flex items-center gap-3">
      ...
      <button onClick={handleSave} className="rounded-full bg-[#7c4f2a] px-5 py-1.5 text-sm text-amber-50 font-medium transition hover:bg-[#643e1f]">
        保存
      </button>
    </div>
  );
    ...
    <main className="min-h-screen bg-[#f5f0e8] text-[#3d2b1a]">
      <div className="mx-auto max-w-2xl px-8 py-10">
        ...
        <div className="space-y-5">
          ...
          <div className={cardClass}>
            ...
            <textarea rows={6} value={settings.modelsText} onChange={e => set("modelsText", e.target.value)}
              ...
              className="w-full resize-none rounded-xl border border-[#e0d4c0] bg-white/70 px-4 py-2.5 text-sm leading-7 text-[#3d2b1a] outline-none placeholder:text-[#c0a078] focus:border-[#b8893f] transition" />
          </div>
        </div>
        ...
        <div className="mt-8 flex items-center justify-between">
          ...
          <button onClick={handleSave} className="rounded-full bg-[#7c4f2a] px-8 py-2.5 text-sm text-amber-50 font-medium transition hover:bg-[#643e1f]">
            保存设置
          </button>
        </div>
