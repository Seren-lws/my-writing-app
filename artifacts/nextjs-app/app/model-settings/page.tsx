...
export default function ModelSettingsPage() {
  ...
  const inputClass =
    "w-full rounded-xl border border-[#e0c9a5] bg-white/70 px-4 py-2.5 text-sm text-[#4f3524] outline-none placeholder:text-[#c7a984] focus:border-[#9b744d] transition";

  const cardClass = "rounded-2xl border border-[#e0c9a5] bg-[#fff8eb] p-6 shadow-sm";
  const labelClass = "mb-2 block text-sm font-medium text-[#6e4b2d]";
      ...
      <div className="mx-auto max-w-2xl px-8 py-12">
        ...
        <div className="mb-10 flex items-start justify-between">
          <div>
            ...
            <h1 className="text-3xl font-semibold text-[#4f3524]">模型设置</h1>
            <p className="mt-2 text-sm text-[#9b744d]">
              配置 AI 中转站地址、密钥和使用的模型。所有内容仅保存在本地。
            </p>
          </div>
        ...
        <div className="space-y-5">
          ...
          <div className={cardClass}>
            ...
            <input
              ...
              className={inputClass}
            />
            <p className="mt-2 text-xs text-[#9b744d]">
              密钥只存储在你的浏览器本地，不会上传到任何服务器。
            </p>
          </div>
